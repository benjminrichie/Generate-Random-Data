...
export default function RandomDataGenerator() {
  ...
  const [fileName, setFileName] = useState("random_data");
    ...
    switch (column.type) {
      case "name":
        if (column.subtype === "surname") {
          return dataBank.surname[
            Math.floor(Math.random() * dataBank.surname.length)
          ];
        } else if (column.subtype === "firstname") {
          return dataBank.firstname[
            Math.floor(Math.random() * dataBank.firstname.length)
          ];
          ...
          const middleName = hasMiddle
            ? dataBank.middlename[
                Math.floor(Math.random() * dataBank.middlename.length)
              ]
            : "";
      ...
      case "location":
        if (column.subtype === "country") {
          return dataBank.country[
            Math.floor(Math.random() * dataBank.country.length)
          ];
        } else if (column.subtype === "city") {
          return dataBank.city[
            Math.floor(Math.random() * dataBank.city.length)
          ];
        }
        return "";
      ...
      case "email":
        ...
        const name =
          namePool[Math.floor(Math.random() * namePool.length)].toLowerCase();
        const domain =
          dataBank.email[Math.floor(Math.random() * dataBank.email.length)];
  ...
  const downloadExcel = () => {
    try {
      ...
      // Generate the Excel file with user-specified name
      ...
      {data.length > 0 && !isGenerating && (
        <div className="mt-4">
          ...
          <p className="mt-4 text-gray-600">
            ...
            {data.length > 10
              ? "Use the download button to get the complete dataset."
              : ""}
          </p>
        </div>
      )}
